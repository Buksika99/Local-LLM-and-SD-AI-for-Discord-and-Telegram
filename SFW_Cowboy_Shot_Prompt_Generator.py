...
def random_clothing_action(random_partial_bottomwear, random_specific_bottomwear, random_topwear, data):
    ...
    if random_specific_bottomwear == "jeans":
...
def handle_camera_angle(random_camera_angle, data):
    composition = data['SFW']["composition"]["camera_angle"]
    camera_focus = data['SFW']["composition"]["camera_focus"]
    random_composition = choose_random_value_from_dict(composition[random_camera_angle])
    if random_composition == "from behind":
        random_camera_focus_from_behind = choose_random_item(camera_focus["from_behind_focus"])
        if random.random() < 0.3:
            return [", ".join(
                [random_camera_angle, random_composition,
                 random_camera_focus_from_behind])]
        else:
            return [", ".join(
                [random_camera_angle, random_composition])]
    else:
        random_camera_focus_from_front = choose_random_item(camera_focus["skimpy_from_front"])
        if random.random() < 0.3:
            return [", ".join(
                [random_camera_angle, random_composition, random_camera_focus_from_front])]
        else:
            return [
                ", ".join([random_camera_angle, random_composition])]


def random_topwear_selection(layer):
    layer_category = random.choice(list(layer.keys()))
    layer_style = random.choice(list(layer[layer_category].keys()))
    layer_item = random.choice(layer[layer_category][layer_style])
    return [layer_category, layer_style, layer_item]
...
def generate_random_topwear_layers(random_specified_first_layer_topwear, random_specified_second_layer_topwear,
                                   ...
                                   topwear_first_layer_dict_key, topwear_second_layer_dict_key, check_if_front_or_back,
                                   data, user_selected_second_topwear_layer=None):
    shirt_actions = data["SFW"]["actions"]["clothing_actions"]["shirt_actions"]
    hoodie_actions = data["SFW"]["actions"]["clothing_actions"]["hoodie_actions"]
    sweater_actions = data["SFW"]["actions"]["clothing_actions"]["sweater_actions"]
    ...
    print(user_selected_second_topwear_layer)
    ...
    if first_layer_open_or_closed == "open" and second_layer_open_or_closed == "open":
        if "from front" in check_if_front_or_back:
...
def generate_prompt_Cowboy_Out_Of_Frame_SFW(data, **kwargs):
    prompt = []
    SFW = data['SFW']
    ...
    # Composition
    camera_angle = "feet out of frame"  # ONLY USE FEET OUT OF FRAME
    random_camera_composition = handle_camera_angle(camera_angle, data)
    check_if_front_or_back = random_camera_composition[0]
    ...
    if user_selected_full_attire is not None:
        random_full_attire = choose_random_item(user_selected_full_attire)
    else:
    ...
    if (random.random() < 0.1) and (("from front" in check_if_front_or_back and random_hand_actions == "") or (
            "from side" in check_if_front_or_back and random_hand_actions == "")):
        prompt.append(choose_random_value_from_dict(actions["lora_actions"]["from_front"]))
    else:
    ...
    if full_attire_or_pieces_of_attire == full_attire:
        ...
        prompt.append(
            generate_bottomwear(" BREAK " + random_bottomwear_type, random_specific_bottomwear,
...
def generated_prompt(**kwargs):
    ...
    generated_prompt_in_function = generate_prompt_Cowboy_Out_Of_Frame_SFW(data, **kwargs)
